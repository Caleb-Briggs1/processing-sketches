...
class map {
 ...
 float[][] walls = new float[int(random(90)) ][4];
 ArrayList<bacteria> food = new ArrayList();
 PImage images;
 void init() {
   images = loadImage("m_sand.png");
 }
 void generateMap() {
 ...
 void drawCostume(float x, float y, int size, int cost) {
  ...
  void initFood() {//creates food
    //for each item, create food based on chase
      float maxDist = dist(width/2,height/2,0,0);
    for (int i = 0; map.length > i; i++){
     
      if ( int(random(0,pow( (dist(map[i][0],map[i][1],width/2,height/2) /maxDist )*15,2.0))) == 0) {
        fill(250,0,0);
  ...
  ArrayList<float[]> infoSight(character cha) {
    float x = cha.x;
    float y = cha.y;
    ...
    for (int i =0; food.size() > i; i++){
      ...
      if ( abs(x - cur.x) < size1 && abs(y-cur.y) < size2) {
        float[] b= {cur.x,cur.y,cur.energy};
        info.add(b);
      }
    }
    return info;
  }
  void evolve(character Char, int choi) {
    float[] evolveData = { 100,200,250,250,500};
...
class bacteria {
  ...
  void update() {
    ...
    y += acel.y;
    acel = acel.mult(.95);
    energy += .5 - dist(x,y,width/2,height/2)/ (dist(0,0,width/2,height));
    energy -= 0.1;
...
class character {
  ...
  void move(map Map ,float ang, float speed_) {
     ...
     if (! hit(Map,x + sin(ang) * speed_, y + cos(ang) * speed_) && speed_ <= speed) {
     x += sin(ang) * speed_;
     y += cos(ang) * speed_;
